...
[!] Enter the name of the file you'd like to create for your wordlist.
    ...
    ans = input("[?] Return to the menu? y/n/q -> ")
    if ans=="y":
        mnu()
    if ans=="n":
...
if __name__ == "__main__":
    try:
        if (len(sys.argv) > 1):
            ...
            if "-c" in cmdargs[0:]:
                    if "-ct" in cmdargs[0:]:
                        wtidx=cmdargs.index("-ct")
                        wtype=cmdargs[wtidx+1]
                        print(wtype)
                    else:
                        sys.stderr.write("[!] Please specify one wordtype to check against. (-ct)\n")
